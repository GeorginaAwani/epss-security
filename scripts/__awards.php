		...
		try {
			...
			while ($record = $query->fetch_assoc()) {
				...
				<tr data-award-id='$mid'>
					<td>
						<div><img class='$mediaClass' data-media-id='$mid' src='{$this->MEDIA_ROOT}files/awards/$filename' alt='$desc'/></div>
					</td>
					<td>
	...
	public function getCards($lid = 0){
		...
		try {
			...
			for($i = 0; $i < $rows; ++$i){
				...
				<a class='award-view-btn flex-fill img-view' href=''><div class='award-img bg-img border overflow-hidden rounded-lg'><img src='{$this->MEDIA_ROOT}files/awards/$filename' class='img-fluid'/></div></a>
